...
eunit-core: compile-for-eunit
	...
	$(REBAR) eunit -r skip_apps='ubf,gdss_ubf_proto,ubf_thrift,thrift,folsom,bear,lager,eper,meck,asciiedoc,edown'

eunit-thrift: compile-for-eunit
	@echo "eunit testing (thrift): $(RELPKG) ..."
	$(REBAR) eunit -r skip_apps='gdss_brick,gdss_client,gdss_admin,cluster_info,partition_detector,congestion_watcher,gmt_util,folsom,bear,lager,eper,meck,asciiedoc,edown'

eqc: compile-for-eqc
	@echo "eqc testing: $(RELPKG) ..."
	$(REBAR) eqc -r qc_opts=3000 skip_apps='folsom,bear,lager,eper,meck,ubf,ubf_thrift'

eqc-thrift: compile-for-eqc
	@echo "eqc testing (thrift): $(RELPKG) ..."
	$(REBAR) eqc -r qc_opts=3000 skip_apps='gdss_brick,gdss_client,gdss_admin,cluster_info,partition_detector,congestion_watcher,gmt_util,folsom,bear,lager,eper,meck'
...
compile-for-eunit:
	...
	$(REBAR) compile -r eunit compile_only=true skip_apps='folsom,bear,meck,ubf,ubf_thrift,thrift'
